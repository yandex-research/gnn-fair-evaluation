...
class Config(TypedDict):
    ...
    # Apply Dimensionality-Scaled Prior (Hvarfner et al., ICML 2024) to GPSampler.
    # Requires sampler_type='GPSampler'.
    use_dsp: NotRequired[bool]
...
def main(
    ...
    *,
    ...
    if lib.get_checkpoint_path(output).exists():
        ...
        sampler_type = config.get('sampler_type', 'TPESampler')
        sampler_cls = (
            lib.optuna.OPTUNA_SAMPLERS.get(sampler_type)  #
            or getattr(optuna.samplers, sampler_type)
        )
        print(sampler_cls)
        sampler = sampler_cls(**config.get('sampler', {}), seed=config['seed'])  # type: ignore
        if config.get('use_dsp', False):
            assert isinstance(sampler, optuna.samplers.GPSampler), (
                "use_dsp requires sampler_type='GPSampler'"
            )
            sampler._log_prior = lib.dsp_log_prior
